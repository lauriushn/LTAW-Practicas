...
io.on('connect', (socket) => {
    ...
    //-- Evento de recepción de mensajes directos
    socket.on('direct message', ({ recipient, message }) => {
        const recipientId = Object.keys(users).find(key => users[key] === recipient);
        if (recipientId) {
            // Enviar el mensaje directo solo al usuario seleccionado
            io.to(recipientId).emit('message', `<strong>Mensaje directo de ${nickname}:</strong> ${message}`);
        } else {
            sendToClient(socket, `El usuario ${recipient} no está disponible.`);
        }
    });
});
